...
function Editar(index) {
  ListaContacto.splice(index, 1, {
    nombre: document.getElementById("nombreEd").value,
    telefono: document.getElementById("telefonoed").value,
    correo: document.getElementById("correoed").value,
    etiqueta: document.getElementById("inputGroupSelect01ed").value,
  });
  mostrarLista();
}
...
document.getElementById("btn").addEventListener("click", function () {
  ...
  if (mostrartabla() === "" && etiqueta() === "") {
    ...
    var filtroo = ListaContacto.filter(
      (perss) =>
        perss.nombre === mostrartabla() || perss.etiqueta === etiqueta()
    );
    filtroo.forEach((flt) => {
      const originalIndex = ListaContacto.findIndex(
        (pers) => pers.nombre === flt.nombre
      );
